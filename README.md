...
本模组大量使用了各 DLC 专属机制，**建议开启全部 DLC**，否则部分内容会缺失或弱化：

- **La Résistance**：情报机构、间谍升级（`create_intelligence_agency`、`operative_slot`、`upgrade_intelligence_agency` 等，国策与决议中多次使用）
- **No Step Back**：中型/重型坦克底盘奖励国策（`has_dlc = "No Step Back"` 分支）
- **By Blood Alone**：大型飞机（战略轰炸机）奖励国策（`has_dlc = "By Blood Alone"` 分支）
- **No Compromise, No Surrender**：故宫地标建筑（`has_dlc = "No Compromise, No Surrender"`）

> 若未安装相关 DLC，模组本体仍可运行（相关分支会自动跳过），但玩法体验会明显缩水，建议完整开启。

### 地图 Mod 兼容性

- 本模组**不新增、不修改任何州、省、补给节点、铁路或战略区域文件**，只使用原版 `608 北京` 州。
- 因此与绝大多数地图/战略区域 Mod 兼容，可放心搭配使用。
- 若与其他 Mod 同时加载，请将本模组放在较后加载位置。

## 简介

《The Eighty: 八十崛起》是一个以北京市第八十中学（望京校区）为题材的架空历史模组。玩家扮演“八十中”，从一所学校出发，走向统一华北、影响世界的道路。当前版本包括：
...
- 学期项目为 **20 天推进制**：每个项目需 20 天完成任务，且同一时间仅可推进一项，需要规划学期内的项目优先级。
...
- `608 北京` 开局归 `B80` 所有，并保留中国各势力的原有核心（`B80` 也拥有全部中国势力核心地区）。
- 北京州为原版增强版数据：人口约 964 万、基础设施 3、民用工厂 2、空港 3、故宫地标（需 NCNS DLC）、海军基地 2。
...
| 角色 | 主要职位 | 特质/说明 |
|---|---|---|
| **任炜东** | 国家元首（开局）、陆军元帅 | `务实的教育掌舵者`：稳定+5%、政治点+10%、科研+5% |
| **罗静卿** | 政治顾问、陆军参谋长 | `年级统筹者`：政治点+15%、稳定+5% |
| **李丁** | 政治顾问、军事理论家 | `数学的严谨`：科研+7%、工厂上限效率+5% |
| **梁吉峰** | 政治顾问、空军参谋长 | `物理实验精神`：科研+5%、工厂效率提升+10%、军工厂生产+5% |
...
| 角色 | 解锁国策 | 主要职位 |
|---|---|---|
| **史嘉奥** | `newfocus_70` 快速突击 | 空军参谋长、军长（`学生公社组织者`，学生公社线国家元首） |
| **黎瑞成** | `newfocus_61` 骑兵坦克 | 总参谋部、军长（装甲/骑兵） |
| **李维思** | `newfocus_62` 步坦协同 | 总参谋部、军长（装甲） |
| **宋思彤** | `newfocus_58` | 总参谋部、军长（炮兵） |
| **黄振宇** | `newfocus_69` 战略轰炸 | 空军参谋长 |

> 四条政治路线各有专属国家元首：**刘易达**（学生共和国/民主线）、**吴文琦**（同盟国民主线）、**罗静卿**（教育处集权线）、**史嘉奥**（学生公社线）。
...
> ⚠️ **本模组涉及现实人物的姓名与肖像，请作者与协作者务必阅读本节。**

### 题材声明

- 本模组为**架空历史与战争题材**的粉丝创作，所有游戏内剧情、事件、国策均为虚构演绎。
- 游戏涉及战争、冲突、政治路线分歧等成人化设定，与现实中的任何人、机构、组织**不存在对应关系**，也**不代表作者任何政治立场**。
- 模组中的“八十中”及其角色是对真实校园题材的艺术化再创作，属幽默戏谑风格，不构成对任何现实教师、学生或学校的评价。

### 公开许可要求

模组中使用以下现实人物相关元素，请务必在公开发布前完成授权：

1. **姓名与肖像**：模组中出现的教师与学生角色（任炜东、罗静卿、李丁、梁吉峰等）使用了现实中真实人物的姓名，并配有基于其本人形象制作的游戏肖像。
2. **发布渠道**：包括但不限于 Steam 创意工坊、GitHub 仓库、公众号/群聊分享等一切公开渠道。
3. **授权内容**：请逐一向涉及的真实人物（或其监护人）确认：
   - 同意在公开平台上展示其姓名；
   - 同意其肖像被用于该模组的游戏立绘与展示图；
   - 同意该作品为**战争、架空政治与成人向题材**。
4. **撤回机制**：任何当事人提出异议时，作者应立即删除相关内容并停止传播。

### 当前授权状态

- [ ] 已取得全部涉及人物（或其监护人）的书面/截图授权
- [ ] 已向当事人说明战争与架空政治题材
- [ ] 已保留授权凭证（聊天记录/邮件截图）备查

> 若暂未完成授权，请在创意工坊描述中标注“纯虚构，角色名仅为致敬”等说明，并尽快补齐授权流程。
...
8. **提交信息**：标题用简短中文描述（如 `学期制：新增期末成绩单事件`）；正文必须附正式小报告——按"机制/数值改动 → 涉及文件 → 校验结果"的格式整理，方便另一人（和 AI）从 `git log` 快速定位改动。
